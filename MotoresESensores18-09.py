...
'''drive_base = DriveBase(motor_esquerdo, motor_direito, wheel_diameter=?, axle_track=?)'''
...
#================================================#
# Classe da FINITE STATE MACHINE
class FSM:
    def __init__(self):
        self.state = None
        self.states = {}
    ...
    def set_state(self, name):
        print("Mudando para estado:", name)
    ...
    def run(self):
        while True:
            ...
            if (sensor_1.reflection() < 20) and (sensor_2.reflection() < 20) and not (sensor_3.reflection() < 20):
                ...
                while not (sensor_2.reflection() < 40):
                    motor_esquerdo.run(-_40_porcento)
                    motor_direito.run(_70_porcento)
                    PararMotores()
            # PID básico
    ...
    def run(self):
        ...
        if sensor_1.color() == Color.GREEN:
            ...
            if sensor_1.color() == Color.BLACK and sensor_3.color() == Color.BLACK:
                ...
                self.fsm.set_state("SEGUIR_LINHA")
            DoisMotoresAngulo(velocidade_padrão, 100)
...
#================================================#
fsm = FSM()
fsm.add_state("SEGUIR_LINHA", SeguirLinha(fsm))
fsm.add_state("VERDE", Verde(fsm))
fsm.add_state("OBSTACULO", Obstaculo(fsm))
fsm.add_state("RESGATE", AreaDeResgate(fsm))
fsm.set_state("SEGUIR_LINHA")
#================================================#
'''
